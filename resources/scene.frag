...
void main() {
  ...
  // Calculate fog
  float camera_distance = distance(vpos, wspos_camera);
  float fog_factor = smoothstep(0, 1, camera_distance / 1000.0);
  p3d_FragColor = mix(p3d_FragColor, p3d_Fog.color, fog_factor);
